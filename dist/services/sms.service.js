...
class BrevoSMSService {
    ...
    async sendSMS(phoneNumber, message) {
        try {
            const response = await axios_1.default.post(`${this.baseURL}/transactionalSMS/sms`, {
     ...
     */
    formatPhoneNumber(phoneNumber, countryCode = '+250') {
     ...
     */
    async sendWithdrawalOTP(userId, registeredPhoneNumber, // This MUST be the phone from user's database record
    amount, currency = 'RWF') {
        try {
     ...
     */
    async verifyWithdrawalOTP(userId, otp, amount) {
        try {
            const otpKey = `withdrawal_${userId}`;
     ...
     */
    async resendWithdrawalOTP(userId, phoneNumber, amount, currency = 'RWF') {
        const otpKey = `withdrawal_${userId}`;
        const storedOTP = this.otpStorage.get(otpKey);
     ...
     */
    async sendTransactionStatusSMS(userId, phoneNumber, transactionType, amount, currency, status) {
        try {
            let message = '';
