...
exports.SecurityInfoCapture = exports.AuthMailingIntegration = void 0;
const brevo_auth_1 = require("../utils/brevo.auth");
class AuthMailingIntegration {
    constructor() {
        this.mailingService = new brevo_auth_1.BrevoMailingService();
        this.companyInfo = {
            name: 'Jambolush',
    ...
    // --- CONTEXT BUILDERS ---
    buildBaseContext(user, securityInfo) {
        return {
            user: {
    ...
    buildVerificationContext(user, code, expiresInMinutes = 10) {
     ...
     */
    async sendPasswordResetOTP(user, otp, securityInfo) {
        try {
     ...
     */
    async sendPasswordChangedNotification(user, securityInfo) {
        try {
     ...
     */
    async sendLoginNotification(user, securityInfo) {
        try {
            const context = this.buildBaseContext(user, securityInfo);
     ...
     */
    async sendSuspiciousActivityAlert(user, securityInfo) {
        try {
            const context = this.buildBaseContext(user, securityInfo);
     ...
     */
    async sendAccountStatusChange(user, status) {
        try {
            const context = this.buildBaseContext(user);
     ...
     */
    async sendProfileUpdateNotification(user, securityInfo) {
        try {
            const context = this.buildBaseContext(user, securityInfo);
     ...
     */
    shouldSendLoginNotification(user, securityInfo) {
        if (!securityInfo)
            return false;
...
exports.SecurityInfoCapture = SecurityInfoCapture;
