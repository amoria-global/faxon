...
class EscrowService {
    constructor() {
        this.escrowCredentials = {};
        this.escrowConfig = {
            baseUrl: config_1.config.escrow.baseUrl || 'https://api.escrowpayments.com',
            apiKey: config_1.config.escrow.apiKey,
            secretKey: config_1.config.escrow.secretKey,
            merchantId: config_1.config.escrow.merchantId,
            environment: (config_1.config.escrow.environment === 'production' ? 'production' : 'sandbox'),
            timeout: 30000,
            retryAttempts: 3,
            webhookSecret: config_1.config.escrow.webhookSecret,
            callbackUrl: config_1.config.escrow.callbackUrl,
            defaultCurrency: 'USD', // USD as default
            supportedCurrencies: ['USD', 'RWF'] // USD and RWF support
        };
        this.escrowClient = axios_1.default.create({
        ...
        this.escrowClient.interceptors.response.use((response) => response, async (error) => {
    ...
    generateSignature(data, timestamp) {
        ...
        return crypto_1.default.createHmac('sha256', this.escrowConfig.secretKey)
            .update(payload)
            .digest('hex');
    }
    // --- ESCROW DEPOSIT (Funding escrow account) ---
    async createEscrowDeposit(userId, data) {
        try {
            // Validate currency
            ...
            const response = await this.escrowClient.post('/v1/escrow/create', escrowRequest);
    ...
    async processEscrowWithdrawal(userId, data) {
        try {
            ...
            const response = await this.escrowClient.post(`/v1/escrow/${escrowTransaction.escrowId}/release`, releaseRequest);
    ...
    async processEscrowTransfer(userId, data) {
        try {
            ...
            // Submit to Escrow API
            const response = await this.escrowClient.post('/v1/escrow/transfer', transferRequest);
            // Update transaction
            const updatedTransaction = await this.updateEscrowTransaction(escrowTransaction.id, {
                escrowId: response.data.escrow_id,
                externalId: response.data.transaction_id,
    ...
    async createP2PEscrowPayment(userId, data) {
        try {
            ...
            // Submit to Escrow API
            const response = await this.escrowClient.post('/v1/escrow/p2p/create', p2pRequest);
    ...
    async getEscrowTransaction(transactionId, userId) {
        try {
    ...
    async getUserEscrowTransactions(userId, status) {
        try {
            const whereClause = {
                OR: [
                    { userId },
    ...
    async disputeEscrowTransaction(transactionId, userId, disputeReason) {
        try {
            const transaction = await prisma.escrowTransaction.findFirst({
                where: {
                    id: transactionId,
            ...
            const response = await this.escrowClient.post(`/v1/escrow/${transaction.escrowId}/dispute`, disputeRequest);
            // Update transaction status
    ...
    // --- WEBHOOK HANDLING ---
    async handleEscrowWebhook(webhookData) {
        try {
            const transaction = await prisma.escrowTransaction.findFirst({
                where: {
                    OR: [
    ...
    async verifyReleasePermission(transactionId, userId) {
    ...
    async notifyRecipient(recipientId, transactionId, amount, currency) {
    ...
    async sendEscrowNotification(userId, transactionId, eventType) {
    ...
    mapEscrowStatusToTransactionStatus(escrowStatus) {
        const statusMap = {
            'pending': 'pending',
            'funded': 'funded',
            'released': 'released',
