...
exports.EnhancedPropertyService = void 0;
...
class EnhancedPropertyService {
    ...
    async getAgentDashboardWithTransactions(agentId) {
        const [totalClientsData, activeClientsData, transactionSummary, recentBookings, monthlyCommissions, escrowTransactions, paymentTransactions] = await Promise.all([
    ...
    async getAgentTransactionSummary(agentId) {
    ...
    async getEscrowCommissionStates(agentId) {
    ...
    async getPaymentCommissionStates(agentId) {
    ...
    async getAgentRecentBookingsWithTransactions(agentId) {
    ...
    async getBookingTransactionData(agentBookingId) {
    ...
    async getAgentMonthlyCommissionsWithTransactions(agentId) {
    ...
    async getAgentEscrowTransactions(agentId) {
    ...
    async getAgentPaymentTransactions(agentId) {
    ...
    async getAgentEarningsWithTransactions(agentId, timeRange) {
        const now = new Date();
        let startDate;
        switch (timeRange) {
            case 'week':
        ...
        const [transactionSummary, escrowBreakdown, paymentBreakdown, withdrawalRequests] = await Promise.all([
            this.getAgentTransactionSummary(agentId),
    ...
    // === WITHDRAWAL REQUESTS ===
    async getAgentWithdrawalRequests(agentId, startDate) {
    ...
    // === ESCROW BREAKDOWN ===
    async getAgentEscrowBreakdown(agentId, startDate) {
        const escrowTransactions = await prisma.escrowTransaction.findMany({
            where: {
                OR: [
                    { userId: agentId },
    ...
    // === PAYMENT BREAKDOWN ===
    async getAgentPaymentBreakdown(agentId, startDate) {
        const paymentTransactions = await prisma.paymentTransaction.findMany({
            where: {
                userId: agentId,
                createdAt: { gte: startDate }
    ...
    async getActiveAgentClients(agentId) {
        return await prisma.agentBooking.groupBy({
            by: ['clientId'],
            where: {
                agentId,
    ...
    async getAgentBookingCount(agentId) {
        return await prisma.agentBooking.count({
            where: { agentId }
        });
    }
    async getPeriodEarnings(agentId, startDate) {
    ...
    async getPeriodBookingCount(agentId, startDate) {
        return await prisma.agentBooking.count({
            where: {
                agentId,
                createdAt: { gte: startDate }
    ...
    async getTransactionMonitoringDashboard(agentId) {
        const [escrowOverview, paymentOverview, recentTransactions, pendingWithdrawals, failedTransactions] = await Promise.all([
    ...
    async getEscrowOverview(agentId) {
    ...
    async getPaymentOverview(agentId) {
    ...
    async getRecentTransactionActivity(agentId) {
    ...
    async getPendingWithdrawals(agentId) {
        return await prisma.withdrawalRequest.findMany({
            where: {
                userId: agentId,
                status: 'PENDING'
    ...
    async getFailedTransactions(agentId) {
    ...
    generateTransactionAlerts(escrowOverview, paymentOverview, failedTransactions) {
        const alerts = [];
        // Check for high failure rates
        const totalEscrow = Object.values(escrowOverview).reduce((sum, item) => sum + item.count, 0);
...
exports.EnhancedPropertyService = EnhancedPropertyService;
