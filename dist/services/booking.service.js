...
const brevo_booking_1 = require("../utils/brevo.booking");
const prisma = new client_1.PrismaClient();
class BookingService {
    constructor() {
        this.emailService = new brevo_booking_1.BrevoBookingMailingService();
    }
    // --- BLOCKED DATES HELPER METHODS ---
    async createBlockedDatesForBooking(propertyId, checkIn, checkOut, bookingId, reason = "booked") {
    ...
    async removeBlockedDatesForBooking(bookingId) {
    ...
    async createPropertyBooking(userId, data) {
        ...
        try {
            await this.createBlockedDatesForBooking(data.propertyId, checkInDate, checkOutDate, booking.id, "confirmed booking");
        }
        catch (blockError) {
    ...
    async getPropertyBookingById(bookingId, userId) {
    ...
    async updatePropertyBooking(bookingId, userId, data) {
        const booking = await prisma.booking.findFirst({
            where: {
                id: bookingId,
                OR: [
        ...
        if (data.status) {
            try {
                if (data.status === 'cancelled') {
                    ...
                    await this.createBlockedDatesForBooking(updatedBooking.propertyId, updatedBooking.checkIn, updatedBooking.checkOut, bookingId, "confirmed booking");
                }
            }
            catch (blockError) {
    ...
    async updateBookingDates(bookingId, userId, newCheckIn, newCheckOut) {
        const booking = await prisma.booking.findFirst({
            where: {
                id: bookingId,
                OR: [
        ...
        try {
            ...
            await this.createBlockedDatesForBooking(booking.propertyId, newCheckIn, newCheckOut, bookingId, "updated booking");
        }
        catch (blockError) {
    ...
    async getPropertyAvailability(propertyId, startDate, endDate) {
        ...
        const isAvailable = !blockedPeriods.some(period => period.start < endDate && period.end > startDate);
        return {
            isAvailable,
            blockedPeriods: blockedPeriods.sort((a, b) => a.start.getTime() - b.start.getTime())
        };
    }
    async searchPropertyBookings(userId, filters, page = 1, limit = 20) {
        ...
        if (filters.minAmount || filters.maxAmount) {
            ...
            if (filters.minAmount)
                whereClause.totalPrice.gte = filters.minAmount;
            if (filters.maxAmount)
                whereClause.totalPrice.lte = filters.maxAmount;
        }
        if (filters.search) {
            whereClause.OR = [
    ...
    async createTourBooking(userId, data) {
    ...
    async getTourBookingById(bookingId, userId) {
    ...
    async updateTourBooking(bookingId, userId, data) {
        const booking = await prisma.tourBooking.findFirst({
            where: {
                id: bookingId,
                OR: [
        ...
        if (data.checkInStatus)
            updateData.checkInStatus = data.checkInStatus;
    ...
    async searchTourBookings(userId, filters, page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        const whereClause = {
            OR: [
                { userId: userId },
    ...
    async createAgentBooking(agentId, data) {
        ...
        if (data.type === 'property') {
            const propertyBookingData = data.bookingData;
            ...
            const tourBookingData = data.bookingData;
    ...
    // --- CALENDAR METHODS ---
    async getUserBookingCalendar(userId) {
    ...
    // --- ANALYTICS METHODS ---
    async getGuestBookingStats(userId) {
        ...
        const favoriteDestinations = Object.entries(destinations)
            ...
            .slice(0, 5)
            .map(([dest]) => dest);
        const upcomingBookings = propertyBookings.filter(b => b.status === 'confirmed' && new Date(b.checkIn) > new Date()).length + tourBookings.filter(b => b.status === 'confirmed' && new Date(b.schedule.startDate) > new Date() // Fix: Now schedule is available
        ).length;
        return {
            totalBookings,
            completedBookings,
    ...
    async getUserWishlist(userId, filters = {}, page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        // Build where clause
        ...
        if (filters.minPrice || filters.maxPrice) {
            ...
            if (filters.minPrice)
                propertyWhere.pricePerNight.gte = filters.minPrice;
            if (filters.maxPrice)
                propertyWhere.pricePerNight.lte = filters.maxPrice;
        }
    ...
    async isInWishlist(userId, type, itemId) {
        if (type === 'tour') {
    ...
    // --- HELPER METHODS ---
    transformToPropertyBookingInfo(booking) {
    ...
    transformToTourBookingInfo(booking) {
        return {
            id: booking.id,
