...
class AuthService {
    constructor() {
        ...
        this.brevoEmailService = new brevo_auth_1.BrevoMailingService();
        this.brevoSMSService = new brevo_sms_auth_1.BrevoSMSService();
    }
    // --- CONTEXT CREATION HELPERS ---
    createMailingContext(user, security, verification) {
        return {
            user: {
            ...
            company: {
                ...
                supportEmail: config_1.config.supportEmail,
                logo: config_1.config.companyLogo
            },
            security,
            verification
        };
    ...
    async sendNotifications(user, notificationType, req, verification, additionalData) {
    ...
    async verifyTwoFactorLogin(email, code, req) {
        const user = await prisma.user.findUnique({
            where: { email }
        });
    ...
    async sendTwoFactorCode(userId, req) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
    ...
    // --- PHONE VERIFICATION ---
    async sendPhoneVerificationCode(userId, req) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
    ...
    // --- EMAIL VERIFICATION ---
    async sendEmailVerification(userId, req) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
    ...
    // --- DOCUMENT MANAGEMENT ---
    async updateDocumentUrl(userId, documentType, documentUrl) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
    ...
    async updateUserProfile(userId, data, req) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
        ...
        if (data.eircode !== undefined)
            updateData.eircode = data.eircode;
        if (data.cep !== undefined)
            updateData.cep = data.cep;
        // Handle preferences
        if (data.preferredCommunication !== undefined)
            updateData.preferredCommunication = data.preferredCommunication;
        ...
        if (data.tourGuideType !== undefined)
            updateData.tourGuideType = data.tourGuideType;
        if (data.nationalId !== undefined)
            updateData.nationalId = data.nationalId;
        if (data.companyTIN !== undefined)
            updateData.companyTIN = data.companyTIN;
    ...
    async setupPassword(email, token, newPassword, req) {
    ...
    // --- ADMIN OPERATIONS ---
    async adminCreateUser(data, req) {
        const existingUser = await prisma.user.findUnique({
            where: { email: data.email }
        });
        if (existingUser) {
    ...
    async adminSuspendUser(userId, reason, req) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
    ...
    async adminActivateUser(userId, req) {
        const user = await prisma.user.findUnique({
            where: { id: userId },
            select: {
                id: true,
    ...
    async adminResetUserPassword(userId, req) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
    ...
    // --- KYC METHODS ---
    async submitKYC(userId, personalDetails, addressDocumentUrl) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
        ...
        if (updatedUser.phone) {
            try {
                await this.brevoSMSService.sendKYCStatusSMS(this.createSMSContext(updatedUser), 'pending_review');
            }
            catch (error) {
    ...
    async adminUpdateKYCStatus(userId, status, req) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
        ...
        if (updatedUser.phone && status !== 'pending') {
            try {
                await this.brevoSMSService.sendKYCStatusSMS(this.createSMSContext(updatedUser), status);
            }
            catch (error) {
    ...
    async getAgentReferrals(agentId, page = 1, limit = 10) {
    ...
    mapUserStatusToReferralStatus(userStatus) {
        switch (userStatus) {
            case 'active':
                return 'Active';
            case 'pending':
    ...
    async updateReferralStatus(userId, newStatus) {
    ...
    async generateAgentReferralCode(agentId) {
        // Validate that the user is an agent
        const agent = await prisma.user.findUnique({
            where: { id: agentId },
    ...
    async getUserStatistics() {
        const [totalUsers, usersByType, usersByStatus, recentRegistrations, kycCompleted, twoFactorEnabled] = await Promise.all([
            prisma.user.count(),
            prisma.user.groupBy({
                by: ['userType'],
                _count: true
    ...
    getDocumentVerificationStatus(user) {
        if (!user.verificationDocument) {
            return 'none';
        }
    ...
    async generateTokens(userId, email, userType) {
        const accessToken = jsonwebtoken_1.default.sign({ userId: userId.toString(), email, userType }, config_1.config.jwtSecret, { expiresIn: '15m' });
        const refreshToken = jsonwebtoken_1.default.sign({ userId: userId.toString(), email, userType, type: 'refresh' }, config_1.config.jwtSecret, { expiresIn: '30d' });
