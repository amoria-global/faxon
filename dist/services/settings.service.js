...
class SettingsService {
     ...
     */
    async updateSettings(userId, updates, req) {
        ...
        if (updates.security && Object.keys(updates.security).length > 0) {
            try {
                await this.authService.sendNotifications(updatedSettings.user, 'profile_update', req);
            }
            catch (error) {
    ...
    async changePassword(userId, data, req) {
        ...
        const isSameAsOld = await bcryptjs_1.default.compare(data.newPassword, user.password);
        if (isSameAsOld) {
     ...
     */
    async sendVerificationCode(userId, type, req) {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
        if (!user) {
    ...
    async getConnectedAccounts(userId) {
        ...
        const connectedAccounts = await Promise.all(accounts.map(async (account) => {
     ...
     */
    async connectGoogleAccount(userId, accessToken, email) {
        try {
     ...
     */
    async disconnectAccountByProvider(userId, provider) {
     ...
     */
    async handleTwoFactorUpdate(userId, enable, req) {
        if (enable) {
    ...
    transformSettings(dbSettings) {
        return {
            ...
            appearance: dbSettings.appearance,
    ...
    async deleteAccount(userId, password, reason) {
        ...
        try {
            await this.authService.sendNotifications(user, 'account_status', null, undefined, {
                status: 'scheduled_for_deletion',
                deletionDate: deletionDate.toISOString(),
                gracePeriodDays: 30
            });
     ...
     */
    async getSettingsSummary(userId) {
        ...
        const notificationsEnabled = notificationKeys.filter(key => notifications[key]).length;
        // Calculate security score
        let securityScore = 0;
        const security = settings.security;
        if (security.twoFactorEnabled)
            securityScore += 25;
        if (security.loginNotifications)
            securityScore += 15;
        if (security.passwordChangeNotifications)
            securityScore += 15;
        if (security.suspiciousActivityAlerts)
            securityScore += 15;
