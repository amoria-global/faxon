...
exports.BrevoPaymentMailingService = void 0;
// src/services/brevo.payment.ts
const axios_1 = __importDefault(require("axios"));
const config_1 = require("../config/config");
// --- PAYMENT MAILING SERVICE CLASS ---
class BrevoPaymentMailingService {
    constructor() {
     ...
     */
    async sendTransactionStatusEmail(context) {
        ...
        const subject = subjectMap[status] || `${typeTitle} Status Update`;
        const emailData = {
     ...
     */
    async sendPaymentMethodAddedEmail(context) {
        if (!context.paymentMethod)
            throw new Error("Payment method context is required.");
        const emailData = {
            sender: this.defaultSender,
            to: [{ email: context.user.email, name: `${context.user.firstName} ${context.user.lastName}` }],
    ...
    getTransactionStatusTemplate(context) {
    ...
    getPaymentMethodAddedTemplate(context) {
    ...
    getTransactionStatusTextTemplate(context) {
    ...
    getPaymentMethodAddedTextTemplate(context) {
...
exports.BrevoPaymentMailingService = BrevoPaymentMailingService;
