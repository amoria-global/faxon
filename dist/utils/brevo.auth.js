...
class BrevoMailingService {
    constructor() {
        try {
            ...
            if (!config_1.config.brevoApiKey) {
                throw new Error('Brevo API key is required but not configured');
            }
            if (!config_1.config.brevoSenderEmail) {
                throw new Error('Brevo sender email is required but not configured');
            }
            // Set API key for transactional emails
            this.transactionalEmailsApi.setApiKey(Brevo.TransactionalEmailsApiApiKeys.apiKey, config_1.config.brevoApiKey);
            // Set API key for contacts
            this.contactsApi.setApiKey(Brevo.ContactsApiApiKeys.apiKey, config_1.config.brevoApiKey);
            this.defaultSender = {
                name: 'Jambolush',
    ...
    logBrevoError(context, error, additionalData) {
        const errorInfo = {
    ...
    handleBrevoError(context, error, additionalData) {
    ...
    async withRetry(operation, context, maxRetries = 3, backoffMs = 1000) {
        let lastError;
    ...
    async createOrUpdateContact(contact) {
        try {
            ...
            await this.withRetry(() => this.contactsApi.createContact(createContactRequest), 'createOrUpdateContact', 3);
        }
        catch (error) {
            const enhancedError = this.handleBrevoError('createOrUpdateContact', error, {
    ...
    async sendTransactionalEmail(emailData) {
        try {
            ...
            const response = await this.withRetry(() => this.transactionalEmailsApi.sendTransacEmail(sendEmailRequest), 'sendTransactionalEmail', 3);
            const messageId = response.messageId || '';
            return messageId;
        }
        catch (error) {
            const enhancedError = this.handleBrevoError('sendTransactionalEmail', error, {
    ...
    async sendTemplateEmail(templateId, context, subject, params) {
        try {
