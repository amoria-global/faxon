...
class BrevoTourMailingService {
     ...
     */
    async sendNewTourConfirmationEmail(context) {
        const emailData = {
     ...
     */
    async sendNewBookingRequestEmail(context) {
        if (!context.booking)
            throw new Error("Booking context is required.");
        const emailData = {
            sender: this.defaultSender,
            to: [{ email: context.recipient.email, name: `${context.recipient.firstName} ${context.recipient.lastName}` }],
     ...
     */
    async sendBookingStatusUpdateEmail(context) {
        if (!context.booking)
            throw new Error("Booking context is required.");
     ...
     */
    async sendNewReviewNotificationEmail(context) {
        if (!context.review)
            throw new Error("Review context is required.");
        const emailData = {
            sender: this.defaultSender,
            to: [{ email: context.recipient.email, name: `${context.recipient.firstName} ${context.recipient.lastName}` }],
    ...
    getNewTourConfirmationTemplate(context) {
    ...
    getNewBookingRequestTemplate(context) {
    ...
    getBookingStatusUpdateTemplate(context) {
        const { recipient, company, tour, booking } = context;
        if (!booking)
            return '';
    ...
    getNewReviewNotificationTemplate(context) {
    ...
    getNewTourConfirmationTextTemplate(context) {
        return `
      New Tour Created!
    ...
    getNewBookingRequestTextTemplate(context) {
        if (!context.booking)
            return '';
        return `
      New Booking Request
    ...
    getBookingStatusUpdateTextTemplate(context) {
        if (!context.booking)
            return '';
    ...
    getNewReviewNotificationTextTemplate(context) {
        if (!context.review)
            return '';
        return `
      New Review Received
...
exports.BrevoTourMailingService = BrevoTourMailingService;
