...
exports.configUtils = exports.config = void 0;
...
exports.config = {
    ...
    pesapal: {
        ...
        baseUrl: process.env.PESAPAL_BASE_URL || (process.env.NODE_ENV === 'production'
...
function validateConfig() {
    ...
    if (!exports.config.pesapal.callbackUrl || !exports.config.pesapal.callbackUrl.includes('http')) {
        errors.push('PESAPAL_CALLBACK_URL must be a valid HTTP/HTTPS URL');
    }
    // Auto-registration specific validation
    if (exports.config.pesapal.autoRegisterIPN) {
        if (!exports.config.pesapal.ipnUrl || !exports.config.pesapal.ipnUrl.includes('http')) {
    ...
    if (exports.config.features.enableJengaPayments) {
    ...
    if (!exports.config.currencies.supported.includes(exports.config.currencies.default)) {
        errors.push(`Default currency ${exports.config.currencies.default} is not in supported currencies list`);
    }
    // Validate split rules
    const { host, agent, platform } = exports.config.escrow.defaultSplitRules;
    ...
    if (exports.config.features.enableEmailNotifications && !exports.config.notifications.email.apiKey) {
        warnings.push('Email notifications enabled but Brevo API key is missing');
    }
    // Production-specific validations
    if (process.env.NODE_ENV === 'production') {
        if (exports.config.security.encryptionKey === 'change-in-production') {
            errors.push('Must set ENCRYPTION_KEY in production environment');
        }
        if (!exports.config.security.corsOrigins.some(origin => origin.includes('jambolush.com'))) {
            warnings.push('No jambolush.com domain in CORS origins for production');
        }
        // Production should use HTTPS URLs
        if (!exports.config.pesapal.callbackUrl.startsWith('https://')) {
    ...
    console.log(`🌍 Supported currencies: ${exports.config.currencies.supported.join(', ')}`);
    console.log(`💰 Default currency: ${exports.config.currencies.default}`);
    console.log(`🔒 Escrow payments: ${exports.config.features.enableEscrowPayments ? 'Enabled (Pesapal)' : 'Disabled'}`);
    console.log(`🔄 Auto IPN registration: ${exports.config.pesapal.autoRegisterIPN ? 'Enabled' : 'Disabled'}`);
    console.log(`🏦 Jenga payments: ${exports.config.features.enableJengaPayments ? 'Enabled' : 'Disabled'}`);
    console.log(`📧 Email notifications: ${exports.config.features.enableEmailNotifications ? 'Enabled' : 'Disabled'}`);
}
...
exports.configUtils = {
    ...
    getCurrencyConfig: (currencyCode) => {
        ...
        return exports.config.currencies[code] || null;
    },
    // Check if feature is enabled
    isFeatureEnabled: (featureName) => {
        return exports.config.features[featureName] === true;
    },
    // Get escrow fee for transaction type
    getEscrowFee: (transactionType) => {
        const fees = exports.config.escrow.fees;
        return fees[transactionType] || 0;
    },
    // Check if currency is supported
    isCurrencySupported: (currencyCode) => {
        return exports.config.currencies.supported.includes(currencyCode.toUpperCase());
    },
    // Environment helpers
    ...
    // Get webhook URLs
    getWebhookUrls: () => ({
        pesapal: exports.config.pesapal.callbackUrl,
        pesapalIpn: exports.config.pesapal.ipnUrl,
        jenga: exports.config.jenga.callbackUrl
    }),
    // Get payment provider settings
    getPaymentProvider: () => ({
        primary: 'pesapal',
        secondary: exports.config.features.enableJengaPayments ? 'jenga' : null,
        escrowEnabled: exports.config.features.enableEscrowPayments,
        autoIPNRegistration: exports.config.pesapal.autoRegisterIPN
    }),
    // Format currency amount based on currency config
    formatCurrency: (amount, currencyCode) => {
        const currencyConfig = exports.configUtils.getCurrencyConfig(currencyCode);
        if (!currencyConfig) {
    ...
    validateTransactionLimits: (amount, type, currency = 'RWF') => {
        const limits = type === 'escrow' ? exports.config.payment.escrowLimits : exports.config.payment.limits;
        const currencyConfig = exports.configUtils.getCurrencyConfig(currency);
        if (!currencyConfig) {
            throw new Error(`Unsupported currency: ${currency}`);
        }
        if (amount < currencyConfig.minAmount) {
            throw new Error(`Amount below minimum for ${currency}: ${exports.configUtils.formatCurrency(currencyConfig.minAmount, currency)}`);
        }
        if (amount > currencyConfig.maxAmount) {
            throw new Error(`Amount exceeds maximum for ${currency}: ${exports.configUtils.formatCurrency(currencyConfig.maxAmount, currency)}`);
        }
        return true;
    },
    ...
    getBusinessHours: () => {
        ...
        const rwandaTime = new Intl.DateTimeFormat('en', {
            timeZone: exports.config.regional.defaultTimezone,
            hour: '2-digit',
            minute: '2-digit',
            hour12: false
        }).format(now);
        ...
        const isWorkingDay = exports.config.regional.businessHours.workingDays.includes(dayName);
        const currentTime = rwandaTime;
        const startTime = exports.config.regional.businessHours.start;
        const endTime = exports.config.regional.businessHours.end;
        ...
        return {
            ...
            workingDays: exports.config.regional.businessHours.workingDays.join(', ')
        };
    },
    // IPN configuration helpers
    getIPNConfiguration: () => ({
        autoRegister: exports.config.pesapal.autoRegisterIPN,
        ipnUrl: exports.config.pesapal.ipnUrl,
        callbackUrl: exports.config.pesapal.callbackUrl,
        cacheDuration: exports.config.pesapal.ipnCacheDuration,
        retryAttempts: exports.config.pesapal.ipnRetryAttempts,
        retryDelay: exports.config.pesapal.ipnRetryDelay
    }),
    // Get all environment URLs for debugging
    getAllUrls: () => ({
        callback: exports.config.pesapal.callbackUrl,
        ipn: exports.config.pesapal.ipnUrl,
        webhook: exports.config.pesapal.callbackUrl,
        client: exports.config.clientUrl,
        base: exports.config.pesapal.baseUrl
    })
};
