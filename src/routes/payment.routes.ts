...
import { escrowIpWhitelist, validateEscrowWebhookSignature, sanitizeEscrowData, logEscrowRequest, rateLimitEscrowPayments, validateEscrowDeposit, validateEscrowWithdrawal, validateEscrowTransfer, validateP2PEscrow } from '../middleware/escrow.middleware';
...
router.post('/webhook/escrow', 
  escrowIpWhitelist,
  validateEscrowWebhookSignature, 
  paymentController.handleEscrowWebhook
);
...
router.post('/deposit/enhanced', 
  rateLimitPayments, 
  sanitizeEscrowData,
  paymentController.enhancedDeposit
);
...
router.post('/transfer/enhanced', 
  rateLimitPayments, 
  sanitizeEscrowData,
  paymentController.enhancedTransfer
);

// === ESCROW PAYMENT OPERATIONS ===
router.use('/escrow', logEscrowRequest); // Log all escrow requests

// --- ESCROW DEPOSIT ---
router.post('/escrow/deposit', 
  rateLimitEscrowPayments,
  validateEscrowDeposit,
  sanitizeEscrowData,
  paymentController.createEscrowDeposit
);

// --- ESCROW WITHDRAWAL (Release) ---
router.post('/escrow/withdraw', 
  rateLimitEscrowPayments,
  validateEscrowWithdrawal,
  sanitizeEscrowData,
  paymentController.processEscrowWithdrawal
);

// --- ESCROW TRANSFER ---
router.post('/escrow/transfer', 
  rateLimitEscrowPayments,
  validateEscrowTransfer,
  sanitizeEscrowData,
  paymentController.processEscrowTransfer
);

// --- PEER-TO-PEER ESCROW ---
router.post('/escrow/p2p', 
  rateLimitEscrowPayments,
  validateP2PEscrow,
  sanitizeEscrowData,
  paymentController.createP2PEscrowPayment
);
...
router.post('/escrow/dispute', 
  rateLimitEscrowPayments,
  sanitizeEscrowData,
  paymentController.createEscrowDispute
);
