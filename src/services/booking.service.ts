...
export class BookingService {
  ...
  async createBooking(guestId: number, data: CreateBookingDto): Promise<BookingInfo> {
  ...
  async updateBooking(bookingId: string, userId: number, data: UpdateBookingDto, userRole: 'guest' | 'host' = 'guest'): Promise<BookingInfo> {
    ...
    if (data.checkIn || data.checkOut) {
      ...
      const validation = await this.validateBooking(
        ...
        newCheckIn,
        newCheckOut,
        data.guests || existingBooking.guests,
        bookingId
      );
  ...
  async cancelBooking(bookingId: string, userId: number, reason: string, userRole: 'guest' | 'host' = 'guest'): Promise<BookingInfo> {
    const whereClause = userRole === 'guest' 
      ? { id: bookingId, guestId: userId }
      : { id: bookingId, hostId: userId };
  ...
  // --- BOOKING QUERIES ---
  async getBookingById(bookingId: string, userId?: number, userRole?: 'guest' | 'host'): Promise<BookingInfo | null> {
  ...
  async searchBookings(filters: BookingSearchFilters, page: number = 1, limit: number = 20) {
    const skip = (page - 1) * limit;
    
    const whereClause: any = {};

    if (filters.propertyId) whereClause.propertyId = filters.propertyId;
    if (filters.guestId) whereClause.guestId = filters.guestId;
    if (filters.hostId) whereClause.hostId = filters.hostId;
    if (filters.status) whereClause.status = filters.status;
    
    if (filters.checkInFrom || filters.checkInTo) {
      whereClause.checkIn = {};
      if (filters.checkInFrom) whereClause.checkIn.gte = new Date(filters.checkInFrom);
      if (filters.checkInTo) whereClause.checkIn.lte = new Date(filters.checkInTo);
    }

    if (filters.checkOutFrom || filters.checkOutTo) {
      whereClause.checkOut = {};
      if (filters.checkOutFrom) whereClause.checkOut.gte = new Date(filters.checkOutFrom);
      if (filters.checkOutTo) whereClause.checkOut.lte = new Date(filters.checkOutTo);
    }

    if (filters.minPrice || filters.maxPrice) {
      whereClause.totalPrice = {};
      if (filters.minPrice) whereClause.totalPrice.gte = filters.minPrice;
      if (filters.maxPrice) whereClause.totalPrice.lte = filters.maxPrice;
    }
  ...
  async getGuestBookings(guestId: number): Promise<BookingSummary[]> {
  ...
  async getHostBookings(hostId: number): Promise<BookingSummary[]> {
    const bookings = await prisma.booking.findMany({
      where: { hostId },
      include: {
        property: { include: { host: true } },
  ...
  async getPropertyBookings(propertyId: number, hostId?: number): Promise<BookingSummary[]> {
  ...
  // --- BOOKING VALIDATION ---
  async validateBooking(propertyId: number, checkIn: string, checkOut: string, guests: number, excludeBookingId?: string): Promise<BookingValidation> {
    const property = await prisma.property.findUnique({
      where: { id: propertyId }
    });
  ...
  // --- DASHBOARD DATA ---
  async getGuestDashboard(guestId: number): Promise<GuestDashboard> {
    const now = new Date();
    ...
    const favoritePropertiesDetails = await Promise.all(
      favoriteProperties.map(async (fav) => {
        const property = await prisma.property.findUnique({
  ...
  async getHostBookingDashboard(hostId: number): Promise<HostBookingDashboard> {
  ...
  private calculatePriceBreakdown(pricePerNight: number, checkIn: string, checkOut: string): PriceBreakdown {
    const checkInDate = new Date(checkIn);
    const checkOutDate = new Date(checkOut);
  ...
  private calculateRefundAmount(booking: any, cancelledBy: 'guest' | 'host'): number {
    const now = new Date();
  ...
  private async processPayment(bookingId: string, data: CreateBookingDto): Promise<void> {
  ...
  private transformToBookingSummary(booking: any): BookingSummary {
    const images = JSON.parse(booking.property.images || '{}');
    const mainImage = this.getMainImage(images);
  ...
  private getMainImage(images: any): string {
