...
// FAQ interfaces (you might want to move these to a separate types file)
interface FAQ {
  id: string;
  question: string;
  answer: string;
...
interface SupportTicket {
  ...
  userId: number;
  assignedTo?: number | null;  // Allow null values
...
const filterAndSort = (items: any[], filters: any) => {
  let filtered = [...items];
...
export const markFAQHelpful = async (req: Request, res: Response) => {
  try {
    const { faqId } = req.params;
...
export const getHelpStats = async (req: Request, res: Response) => {
  try {
